...
class ToolBar(tk.Frame):
	def __init__(self,master=None):
		...
		self.openButton = tk.Button(self , image=self.openImg, border=0, bg="white" , command=lambda : File_Explorer.browse_file(master))
		...
		self.scaleButton = tk.Button(self , image=self.scaleImg, border=0, bg="white" , command=lambda : self.master.set_cursor("fleur"))
		self.scaleButton.grid(row=0 ,column=2,padx=5,pady=3)
	
		self.zoomInImg = tk.PhotoImage(file="images/zoomIn.png")
		self.scaleButton = tk.Button(self , image=self.zoomInImg, border=0, bg="white" , command=lambda : self.master.set_cursor("plus"))
